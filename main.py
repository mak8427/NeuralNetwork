...
class optimizer_adagrad:
    ...
    def pre_update_paramns(self):
        if self.decay:
            self.current_learning_rate = self.lerning_rate * (1. / (1. + self.decay * self.iterations))
...
class optimizer_RMS:
    def __init__(self, decay=0., learning_rate=0.001, epsilon=1e-7, rho=0.9):
        self.decay = decay
        self.learning_rate = learning_rate
        self.current_learning_rate = learning_rate
        self.epsilon = epsilon
...
for epoch in range(0, 10000):
    ...
    # Print accuracy
    if not epoch % 100:
        print(str(epoch) + '  ' + str(accuracy) + '  ' + str(loss) + '  ' + str(optimizer.current_learning_rate))
